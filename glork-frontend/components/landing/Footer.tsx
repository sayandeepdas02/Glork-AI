...
export default function Footer() {
  return (
    <footer className="relative overflow-hidden border-t border-white/10">
      ...
      />
      {/* Dark overlay — slightly heavier than hero for denser footer content */}
        ...
        <div className="px-8 lg:px-14 py-10 grid grid-cols-1 md:grid-cols-5 gap-10 lg:gap-12 border-b border-white/10">
          ...
          <div className="md:col-span-2 flex flex-col items-start">
            <Link href="/" className="flex items-center gap-2 mb-5">
              ...
              <span className="text-[14px] text-white">Glork</span>
            </Link>

            <p className="text-[13px] text-white/60 leading-[1.65] max-w-[260px] mb-6">
            ...
            <div className="flex items-center gap-2">
              {[
                ...
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="flex h-7 w-7 items-center justify-center rounded border border-white/20 bg-white/10 text-white/60 hover:text-white hover:border-white/40 hover:bg-white/15 transition-all duration-150"
                >
          ...
          {Object.entries(footerLinks).map(([group, links]) => (
            <div key={group}>
              <h4 className="text-[10.5px] text-white/40 uppercase tracking-[0.2em] mb-4">
                {group}
              </h4>
              <ul className="space-y-2.5">
                {links.map((l) => (
                  <li key={l}>
                    <a href="#" className="text-[13px] text-white/65 hover:text-white transition-colors duration-150">
                      {l}
                    </a>
                  </li>
                ))}
