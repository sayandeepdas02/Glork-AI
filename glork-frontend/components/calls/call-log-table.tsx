    ...
    <div className="rounded-2xl bg-[#141210] border border-white/6 overflow-hidden">
      ...
      {Array.from({ length: 6 }).map((_, i) => (
        <div key={i} className="flex items-center gap-4 px-5 py-4 border-b border-white/5">
          ...
          <div className="h-3.5 w-10 rounded bg-white/5 animate-pulse" />
        </div>
      ))}
    </div>
  )
    ...
    <div className="rounded-2xl bg-[#141210] border border-white/6 overflow-hidden">
      ...
      <div className="divide-y divide-white/5">
        {calls.map((call) => (
          <div
            key={call.id}
            className="grid grid-cols-[minmax(0,1fr)_auto] md:grid-cols-[minmax(0,1.5fr)_minmax(0,1.5fr)_80px_minmax(0,1fr)_minmax(0,2fr)_60px] gap-4 items-center px-5 py-4 hover:bg-white/4 transition-colors group"
          >
            {/* Caller */}
            <div className="flex items-center gap-3 min-w-0">
              <div className="h-9 w-9 shrink-0 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center group-hover:bg-[#FF5500]/10 group-hover:border-[#FF5500]/20 transition-colors">
                <Phone className="h-4 w-4 text-[#8A8480] group-hover:text-[#FF7733] transition-colors" />
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold text-white font-mono tabular-nums">
                  {call.caller_phone ? formatPhone(call.caller_phone, true) : (
                    <span className="text-[#8A8480] font-sans text-xs">Unknown</span>
                  )}
                </p>
                {/* Mobile: date below */}
                {call.started_at && (
            ...
            {call.started_at ? (
              <div className="hidden md:block">
                <p className="text-sm font-medium text-[#F5F0EB]">{formatAppointmentDate(call.started_at)}</p>
            ...
            {/* Summary */}
            <div className="hidden md:block text-xs text-[#8A8480] leading-relaxed">
              {call.agent_summary ? (
            ...
            <div className="hidden md:flex justify-end">
              <Link
                href={`/calls/${call.id}`}
                className="text-xs font-semibold text-[#FF7733] hover:text-[#FF5500] transition-colors opacity-0 group-hover:opacity-100"
              >
                View →
              </Link>
            </div>
            ...
            {/* Mobile link */}
            <Link href={`/calls/${call.id}`} className="md:hidden text-xs font-semibold text-[#FF7733]">
              View →
            </Link>
          </div>
        ))}
